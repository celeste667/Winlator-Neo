...
public abstract class WineUtils {
    public static void createDosdevicesSymlinks(Container container, boolean addDriveCDRom) {
        ...
        // Drive K: aponta sempre para o diretório de componentes instaláveis do Winlator.
        // Isso permite que qualquer container acesse os componentes via K:\ sem configuração manual.
        File installedComponentsDir = new File(Container.INSTALLED_COMPONENTS_PATH);
        if (!installedComponentsDir.isDirectory()) {
            installedComponentsDir.mkdirs();
            FileUtils.chmod(installedComponentsDir, 0771);
        }
        // Remove symlink anterior (se existir) e recria apontando para installed_components
        new File(dosdevicesPath + "/k:").delete();
        FileUtils.symlink(installedComponentsDir.getAbsolutePath(), dosdevicesPath + "/k:");
    }
