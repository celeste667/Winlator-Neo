...
import com.winlator.core.SaveBackupManager;
...
public class ContainersFragment extends Fragment {
    ...
    private class ContainersAdapter extends RecyclerView.Adapter<ContainersAdapter.ViewHolder> {
            ...
            listItemMenu.setOnMenuItemClickListener((menuItem) -> {
                switch (menuItem.getItemId()) {
                    ...
                    case R.id.menu_item_backup_saves:
                        confirmAndBackup(container);
                        break;
                    case R.id.menu_item_restore_saves:
                        confirmAndRestore(container);
                        break;
                }
                return true;
            });
        ...
        private void confirmAndBackup(Container container) {
            String name = container.getName();
            new AlertDialog.Builder(getContext())
                .setTitle(R.string.backup_saves_title)
                .setMessage(getString(R.string.backup_saves_message, name))
                .setPositiveButton(R.string.backup_saves_confirm, (d, w) -> {
                    preloaderDialog.show(R.string.loading);
                    new Thread(() -> {
                        int count = SaveBackupManager.backup(container);
                        getActivity().runOnUiThread(() -> {
                            preloaderDialog.close();
                            String msg = count >= 0
                                ? getString(R.string.backup_success, count, name)
        ...
        private void confirmAndRestore(Container container) {
            String name = container.getName();
            if (!SaveBackupManager.hasBackup(container)) {
                Toast.makeText(getContext(),
                    getString(R.string.no_backup_found, name),
                    Toast.LENGTH_LONG).show();
                return;
            }
            new AlertDialog.Builder(getContext())
                .setTitle(R.string.restore_saves_title)
                .setMessage(getString(R.string.restore_saves_message, name))
                .setPositiveButton(R.string.restore_saves_confirm, (d, w) -> {
                    preloaderDialog.show(R.string.loading);
                    new Thread(() -> {
                        int count = SaveBackupManager.restore(container);
                        getActivity().runOnUiThread(() -> {
                            preloaderDialog.close();
                            String msg = count >= 0
